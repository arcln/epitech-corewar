...
** Last update Tue Apr 25 11:11:14 2017 Arthur Chaloin
*/
  ...
  if (error_len > 0 && (message = malloc(sizeof(char) * (error_len + 1))))
    {
      ...
      my_dprintf(2, message);
      free(message);
    }
  else if (error_len > 0)
    my_dprintf(2, "Shaders: An error occured with a shader\n");
}
...
static void	error_program(GLuint program_id)
{
  ...
  if (error_len > 0 && (message = malloc(sizeof(char) * (error_len + 1))))
    ...
    my_dprintf(2, "Shaders: An error occured with a program\n");
}
			     ...
			     const char *fragment_file)
{
  ...
  if (load_shader(vertex_file, vertex_id) == 84)
    my_dprintf(2, "Could not read the vertex shader '%s'\n", vertex_file);
  if (load_shader(fragment_file, fragment_id) == 84)
    my_dprintf(2, "Could not read the fragment shader '%s'\n", fragment_file);
