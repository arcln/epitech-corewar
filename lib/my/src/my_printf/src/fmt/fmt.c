...
** Last update Sun Mar 26 03:10:09 2017 Thomas ARBONA
*/
...
static void	init_fmt_data(t_fmt_data *fmt_data)
{
  ...
  if (!(fmt_data->buff = malloc(sizeof(char) * BUFF_SIZE)))
    return;
  my_memset(fmt_data->buff, '0', BUFF_SIZE);
  fmt_data->buff_cur = 0;
  fmt_data->cursor = 0;
  fmt_data->index = 0;
  fmt_data->on_specs = 0;
...
static char	*push_buffer(t_fmt_data *fmt_data, char *str)
{
  ...
  while (fmt_data->buff_cur + my_strlen(str) >= my_strlen(fmt_data->buff))
	...
	return (NULL);
      ...
      my_memset(tmp_buff + my_strlen(fmt_data->buff), '0',
		buff_len - my_strlen(fmt_data->buff));
